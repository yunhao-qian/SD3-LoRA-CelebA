...
![Uncurated Generated Images from Our Fine-Tuned Model with NCUT Visualizations](./images/generated_images_and_ncut_visualizations.png)

In this project, we fine-tuned Stable Diffusion 3 Medium on the CelebA-HQ and CelebA datasets using prompts generated by BLIP-2 and Llama 3, with Low-Rank Adaptation (LoRA). Fine-tuning decisions were guided by mean ablation experiments, and improvements were validated using Fréchet Inception Distance (FID) scores. By leveraging the model's joint attention mechanism, we identified token affinities across modalities and images. These affinities were visualized through heatmaps and Normalized Cuts (NCUT) clustering. This project was completed as part of CIS 6800: Advanced Topics in Machine Perception at the University of Pennsylvania.
